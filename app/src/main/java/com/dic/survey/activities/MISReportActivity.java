...
public class MISReportActivity extends AppCompatActivity {
    ...
    private void buildReports(List<SurveyResponse> all) {
        ...
        // Stat cards row 1
        addStatRow(
            "Total Responses", String.valueOf(total), "#1A4B8C",
            "Synced to Server", String.valueOf(synced), "#4CAF50"
        );
        addStatRow(
            "Submitted (Pending Sync)", String.valueOf(submitted), "#2196F3",
            "Drafts", String.valueOf(drafts), "#FF9800"
        );
        addStatRow(
            "Last 7 Days", String.valueOf(last7days), "#9C27B0",
        ...
        int completed = synced + submitted;
        addProgressCard("Submitted + Synced", completed, total, "#4CAF50");
    ...
    private void addSectionHeader(String title) {
        ...
        div.setBackgroundColor(Color.parseColor("#1A4B8C"));
        div.setAlpha(0.2f);
    ...
    private void addStatRow(String l1, String v1, String c1,
                             String l2, String v2, String c2) {
        ...
        row.addView(spacer);
        row.addView(makeStatCard(l2, v2, c2));
    ...
    private void addBarChart(String[] labels, String[] icons, int[] counts, int[] colors, int total) {
        ...
        for (int i = 0; i < labels.length; i++) {
            ...
            LinearLayout.LayoutParams llp = new LinearLayout.LayoutParams(dp(110), -2);
            ...
            LinearLayout.LayoutParams blp = new LinearLayout.LayoutParams(0, -1, pct > 0 ? pct : 0);
    ...
    private void addProgressCard(String label, int value, int total, String colorHex) {
        ...
        pctTv.setText(value + "  (" + pct + "%)");
        pctTv.setTextSize(13f);
        ...
        barBg.setLayoutParams(blp);

        View bar = new View(this);
        bar.setBackgroundColor(Color.parseColor(colorHex));
        bar.setLayoutParams(new LinearLayout.LayoutParams(0, -1, pct > 0 ? pct : 0));
        barBg.addView(bar);
        View barRem = new View(this);
        barRem.setLayoutParams(new LinearLayout.LayoutParams(0, -1, 100 - pct));
        barBg.addView(barRem);
    ...
    private void addDistrictRow(String district, int count, int total) {
        ...
        View bar = new View(this);
        bar.setBackgroundColor(Color.parseColor("#1A4B8C"));
        bar.setLayoutParams(new LinearLayout.LayoutParams(0, -1, pct > 0 ? pct : 0));
        barBg.addView(bar);
    ...
    private void addSyncStatusTable(int synced, int submitted, int drafts, int total) {
        ...
        table.setPadding(dp(16), dp(14), dp(16), dp(14));
        table.setBackgroundColor(Color.WHITE);

        addTableRow(table, "Status", "Count", "% of Total", true);
        addTableRow(table, "✅ Synced to Server", String.valueOf(synced),
            pct(synced, total) + "%", false);
        addTableRow(table, "📤 Submitted (Pending)", String.valueOf(submitted),
            pct(submitted, total) + "%", false);
        addTableRow(table, "📝 Drafts", String.valueOf(drafts),
            pct(drafts, total) + "%", false);
    ...
    private void addTableRow(LinearLayout parent, String col1, String col2,
                              String col3, boolean isHeader) {
        LinearLayout row = new LinearLayout(this);
        row.setOrientation(LinearLayout.HORIZONTAL);
        ...
        int color = isHeader ? Color.parseColor("#1A4B8C") : Color.parseColor("#424242");
        int style = isHeader ? android.graphics.Typeface.BOLD : android.graphics.Typeface.NORMAL;

        TextView t1 = new TextView(this); t1.setText(col1);
        t1.setTextSize(13f); t1.setTextColor(color); t1.setTypeface(null, style);
        t1.setLayoutParams(new LinearLayout.LayoutParams(0, -2, 2f));
        t1.setPadding(dp(6), 0, 0, 0);

        TextView t2 = new TextView(this); t2.setText(col2);
        t2.setTextSize(13f); t2.setTextColor(color); t2.setTypeface(null, style);
        t2.setLayoutParams(new LinearLayout.LayoutParams(0, -2, 1f));
        t2.setGravity(android.view.Gravity.CENTER);

        TextView t3 = new TextView(this); t3.setText(col3);
        t3.setTextSize(13f); t3.setTextColor(color); t3.setTypeface(null, style);
        t3.setLayoutParams(new LinearLayout.LayoutParams(0, -2, 1f));
        t3.setGravity(android.view.Gravity.END);
        t3.setPadding(0, 0, dp(6), 0);
    ...
    private void addActivityRow(String type, String district, String status, String date) {
        ...
        switch (status) {
            ...
            default: badgeColor = Color.parseColor("#FF9800"); break;
        }
        badge.setBackgroundColor(badgeColor);

        TextView dateTv = new TextView(this);
        dateTv.setText("  " + date);
        dateTv.setTextSize(10f);
    ...
    private int pct(int val, int total) {
